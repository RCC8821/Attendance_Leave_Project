...
//     const today = new Date().toISOString().split('T')[0]; // Default to today: 2025-09-24

//     // Optionally override for testing with sheet data (e.g., 2025-09-23)
//     // const today = "2025-09-23"; // Uncomment for testing with existing sheet data
...
//     // Fetch from API
//     const url = `http://localhost:5000/api/attendance?email=${encodeURIComponent(email)}&date=${today}`;
...
//         setFilteredEmails(usersWithEmail); // Initialize with all emails
...
//         name: user.name,
//         empCode: user.empCode,
//         site: user.site, // Auto-fill site
//         entryType: "",
//         workShift: "",
//         locationName: "",
//         image: null,
//       }));
//       setFilteredEmails(userData); // Reset to all emails
...
//     if (searchTerm === "") {
//       setFilteredEmails(userData); // Show all emails when input is empty
//     } else {
...
//     setFilteredEmails(userData); // Show all emails on focus
//     setIsDropdownOpen(true);
//   };

//   const handleInputBlur = () => {
//     // Increase timeout to allow click to register
...
//     setIsDropdownOpen(true); // Keep dropdown open for site search
//   };
...
//             <div className={`p-4 rounded-xl border-2 transition-all ${attendanceStatus.hasCheckedIn ? 'border-green-200 bg-green-50' : 'border-gray-200 bg-gray-50'}`}>
//               <div className="flex items-center space-x-3">
//                 <CheckCircle className={`w-6 h-6 ${attendanceStatus.hasCheckedIn ? 'text-green-600' : 'text-gray-400'}`} />
//                 <div>
//                   <p className="font-semibold text-sm">Check In</p>
//                   <p className={`text-xs ${attendanceStatus.hasCheckedIn ? 'text-green-600' : 'text-gray-500'}`}>
//                     {attendanceStatus.hasCheckedIn ? 'Completed' : 'Pending'}
//                   </p>
//                 </div>
//               </div>
//             </div>
//             <div className={`p-4 rounded-xl border-2 transition-all ${attendanceStatus.hasCheckedOut ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-gray-50'}`}>
//               <div className="flex items-center space-x-3">
//                 <XCircle className={`w-6 h-6 ${attendanceStatus.hasCheckedOut ? 'text-red-600' : 'text-gray-400'}`} />
//                 <div>
//                   <p className="font-semibold text-sm">Check Out</p>
//                   <p className={`text-xs ${attendanceStatus.hasCheckedOut ? 'text-red-600' : 'text-gray-500'}`}>
...
//                           <div
//                             key={user.email}
//                             onMouseDown={() => handleEmailSelect(user.email)} // Use onMouseDown to capture click before blur
...
//                           <div
//                             key={site}
//                             onMouseDown={() => handleSiteSelect(site)} // Use onMouseDown for site selection
//                             className="px-4 py-3 cursor-pointer hover:bg-indigo-50 transition-colors border-b border-gray-100 last:border-b-0"
//                           >
//                             {site}
//                           </div>
...
function AttendanceForm() {
  ...
  const handleEmailSearch = (e) => {
    ...
    if (searchTerm === "") {
      setFilteredEmails(userData);
      const storedEmail = localStorage.getItem('userEmail');
      if (storedEmail && !formData.email && userData.length > 0) {
        const user = userData.find((u) => u.email && u.email.toLowerCase() === storedEmail.toLowerCase());
        if (user) {
          setFormData((prev) => ({
            ...prev,
            email: user.email,
  ...
  const handleEmailFocus = () => {
    ...
    if (!formData.email) {
      const storedEmail = localStorage.getItem('userEmail');
      if (storedEmail && userData.length > 0) {
        const user = userData.find((u) => u.email && u.email.toLowerCase() === storedEmail.toLowerCase());
        if (user) {
          setFormData((prev) => ({
            ...prev,
